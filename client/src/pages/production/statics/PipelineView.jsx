          ...
          <div className="flex items-center gap-2.5">
            ...
            {!creative.parent_creative_id && (
              <div className="flex items-center gap-2.5">
                ...
                {/* Engine badge — shows which engine generated this creative
                    so the operator can see at a glance. Refines/variants use
                    the same engine the original was made with. */}
                {creative.image_engine && (
                  <span
                    className={`text-[9px] font-mono font-semibold px-1.5 py-0.5 rounded ${
                      creative.image_engine === 'openai'
                        ? 'bg-emerald-500/10 text-emerald-300 border border-emerald-500/20'
                        : 'bg-yellow-500/10 text-yellow-300 border border-yellow-500/20'
                    }`}
                    title={creative.image_engine === 'openai' ? 'Generated by OpenAI' : 'Generated by NanoBanana'}
                  >
                    {creative.image_engine === 'openai' ? 'OAI' : 'NB'}
                  </span>
                )}
              </div>
            )}
