...
import { generateMetaAds, languages, countries } from '../../utils/mockData';
import { toLocalDateStr } from '../../utils/dateUtils';

const initialAds = generateMetaAds(40);
...
export default function MetaDiscovery() {
  ...
  const filtered = useMemo(() => {
    ...
    if (filters.platform) {
      result = result.filter((ad) => ad.platform === filters.platform);
    }
    ...
    if (filters.dateRange !== 'all') {
      ...
      if (days) {
        const cutoff = toLocalDateStr(new Date(now - days * 86400000));
  ...
  return (
    <div className="space-y-6">
      {/* Page header */}
      <div>
        <h1 className="text-xl font-bold text-text-primary">Meta Ad Discovery</h1>
        <p className="text-sm text-text-muted mt-1">
          Discover and analyze ads running on Facebook and Instagram
        </p>
      </div>
