...
export default function BrandDetail({ apiBaseUrl, brandId, onBack }) {
  ...
  const toggleSelectAll = () => setSelectedIds(allSelected ? new Set() : new Set(ads.map((a) => a.id)));
          ...
          {adsLoading ? (
            ...
            <table className="min-w-[1300px] w-full border-collapse">
              <thead className="sticky top-[45px] z-10 bg-bg-elevated">
                <tr>
                  {col('checkbox') && <th style={{ width: 36 }} className="px-2 py-2.5"><input type="checkbox" checked={allSelected} onChange={toggleSelectAll} className="accent-accent" /></th>}
                  ...
                  {col('rank21d') && <th style={{ width: 75 }} className="px-2 py-2.5 text-center text-[10px] uppercase tracking-wider text-text-faint font-normal">21D</th>}
                  {col('rank7d')  && <th style={{ width: 75 }} className="px-2 py-2.5 text-center text-[10px] uppercase tracking-wider text-text-faint font-normal">7D</th>}
                  {col('rank3d')  && <th style={{ width: 75 }} className="px-2 py-2.5 text-center text-[10px] uppercase tracking-wider text-text-faint font-normal">3D</th>}
                  {col('now')     && <th style={{ width: 75 }} className="px-2 py-2.5 text-center text-[10px] uppercase tracking-wider text-text-muted font-semibold bg-bg-hover">NOW</th>}
                  {col('active')  && <th style={{ width: 60 }} className="px-2 py-2.5 text-center text-[10px] uppercase tracking-wider text-text-faint font-normal">ACTIVE</th>}
                  {col('tier')    && <th style={{ width: 90 }} className="px-2 py-2.5 text-center text-[10px] uppercase tracking-wider text-text-faint font-normal">TIER</th>}
                  {col('v7d')     && <th style={{ width: 60 }} className="px-2 py-2.5 text-center text-[10px] uppercase tracking-wider text-text-faint font-normal">V7D</th>}
                  {col('v21d')    && <th style={{ width: 60 }} className="px-2 py-2.5 text-center text-[10px] uppercase tracking-wider text-text-faint font-normal">V21D</th>}
                </tr>
              </thead>
              <tbody>
                {ads.map((ad, i) => (
