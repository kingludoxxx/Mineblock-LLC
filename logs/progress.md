...
TESTED: node server/tests/migrations/migrations.mjs, red first per item (rename re-executed; --strict unknown argument; A8.3 column "type" of relation "creative_analysis" does not exist; A8.1 13 divergent columns; dry-run exit 0 with 107 pending; --allow-pending unknown; mark-applied 001|null,002|null,003|null,004|1), then green on the identical harness: 76 passed, 0 failed, 0 skipped at 2db19fe (baseline at 3caf382: 57/0/0). Hand rehearsal on lane_a2_rehearsal TEMPLATE mineblock_copy with the npm scripts. Failure paths: rename refusal, STRICT orphan refusal, --allow-pending without --dry-run refused, 122's first attempt failed visibly (malformed array literal, fixed with ::text), 122 on a non-empty uuid-shaped table declines with a WARNING and keeps its row.
OUTPUT: rehearsal: dry-run exit 1 (pending 3) → --allow-pending exit 0 → migrate: Backfilled 104 legacy rows, ran 120/121/122, applied 107 | pending 0 | mismatches 0 | orphans 0, exit 0 → dry-run + STRICT dry-run exit 0 → ledger 107 rows, 0 NULL, applied_order 1..107, legacy executed_at untouched → second run "All migrations are up to date", ledger md5 87baacfbdd326f20aeddccd21e829238 identical. creative_analysis shape md5 598824978ec9d4b0237d96d2343f9d3a cols=43 identical for fresh, rehearsal and mineblock_copy. mineblock_copy untouched. Full verbatim outputs: ~/tasks/multistore-hub/briefs/out/PROOF-LANE-A.md, A2 section.
DECISIONS: DECISION MADE: 122 section B declines with RAISE WARNING (not an exception) on a non-empty 016-shaped table. DECISION MADE: boot's report.clean still ignores orphans; they refuse at the pre-deploy step under STRICT. DECISION MADE: no --forget-orphan. DECISION MADE: P2-3 landed inside the P1-1 commit (same mechanism). Known gap: legacy (NULL-checksum) orphans are only caught by STRICT.
STATUS: COMPLETE (local; nothing pushed, nothing deployed, no live call)
---
TIMESTAMP: 2026-09-10 16:40
TASK: Lane E — S1-4 hub SSO exchange (POST /api/v1/hub-sso/exchange), flag off
BUILT: server/src/routes/hubSso.js (HMAC-SHA256 over exact bytes with HUB_SSO_SECRET, STORE_CODE audience, exp 30 s skew + 120 s ceiling, nonce burned in hub_sso_used_tickets in one transaction with the JIT user/role/audit rows, relative-only next, authRateLimiter, no outbound call), server/src/services/hubSession.js (mirrors authController.login cookies/tokens/session; authController untouched), server/migrations/124_hub_sso.sql (hub_sso_used_tickets, hub_role_map), one mount line in routes/index.js, .env.example keys, tests server/tests/hub-sso/{hub-sso,e2e-hub}.mjs.
TESTED: Red first (ERR_MODULE_NOT_FOUND hubSso.js), then 13 fail (real authRateLimiter after 25 refusals from one IP; harness now varies X-Forwarded-For and asserts the 429), then 1 fail (wrong test case: foreign signature over non-JSON bytes is a 401 by design), then 75/75. E2E with the real store-hub app: 19/19; SKIP path exit 0. Edge cases: replay + 4-way race, exp -31/-29/+600 s, wrong store, forged/tampered/malformed/incomplete tickets, secret/STORE_CODE unset, 5 bad next values, inactive user, unknown hub role, existing user keeps role, hub closed → own login answers, stdout+stderr captured: no secret/nonce/signature/token.
OUTPUT: `75 passed, 0 failed` exit 0; `19 passed, 0 failed` exit 0; external grep of the run log for the secret prefix and ticket-shaped strings: 0 and 0. Proof: ~/tasks/multistore-hub/briefs/out/PROOF-LANE-E.md. Commit 3772a4a on day1/lane-sso.
DECISIONS: DECISION MADE — migration 124 appended at the END of Lane A's order.json (not edited here). DECISION MADE — hub_role_map adds operator/owner + '*' fallback (Viewer) to the brief's viewer/editor/admin; missing role → 403, no user. DECISION MADE — Redis session-cache write not mirrored (DB fallback identical). DECISION MADE — JIT users get a random bcrypt-hashed password, email_verified=true.
STATUS: COMPLETE
---
...
BUILT: Worktree /Users/ludo/wt-lane-sso, branch day1/lane-sso rebased onto hub/main 95fc741 (one conflict in logs/progress.md, resolved keeping both entries). Migration 124_hub_sso.sql renamed to 126_hub_sso.sql (123-125 left to Lane C) and appended to server/migrations/order.json (entry 108/108) per docs/MIGRATIONS.md. server/src/routes/hubSso.js: the HUB_SSO_ENABLED check now runs before anything else (404 while dark, no bucket, no read); the route keeps its OWN per-IP bucket that counts refusals only (25/15 min) instead of the shared authRateLimiter, so neither a dark probe nor a burst of valid hops can lock the store's own /auth/login and a 302 is never a failed attempt; HUB_SSO_SECRET under 32 bytes refuses to serve (503, before any HMAC); new required env HUB_ORIGIN (comma-separated list allowed) with an Origin/Referer check (403 otherwise, 503 when unset); a locked account is refused with 423, an email matching more than one users row is a 409, and a successful hop sets last_login and clears the failed-login counters. server/src/services/hubSession.js: the access token now carries hub_sso + sid (the sessions row id), plus peekHubSsoClaims() and hubSessionIsLive(). server/src/middleware/auth.js (+4 lines, announced in COORDINATION.md): a hub-SSO token skips the Redis session cache (read and write) and its sessions row is re-checked every request. New suite server/tests/hub-sso/review-fixes.mjs (43 checks). Commits 8b2cf9d (+ this log entry) on day1/lane-sso; NOT merged into hub/main, nothing pushed, nothing deployed, flag off.
TESTED: Red first on the identical command for every item (HUB_SSO_TEST_DB=...e2_test node server/tests/hub-sso/review-fixes.mjs): dark 26th POST 429 and the store's own login 429 from that IP; 26th valid hop 429; foreign Origin 302; no Origin and no Referer 302; 31-byte secret 302; locked account 302; last_login null; duplicate-case email 302; revoked session still 200 on the next request. Then green on the identical command: 43/43. Migration red (runner: "present ... but NOT listed in order.json: 126_hub_sso.sql") then green (108 applied on an empty database, dry-run 0 pending / 0 mismatches, second run idempotent, ledger applied_order 108). Edge cases: HUB_ORIGIN not a URL -> 503; unparseable Origin header -> 403 (no throw); forged hub_sso token with no sid -> 401; sid naming no row -> 401; unmarked token for the same user -> 200; peekHubSsoClaims on malformed input -> null.
OUTPUT: hub-sso.mjs 75 passed 0 failed; review-fixes.mjs 43 passed 0 failed; e2e-hub.mjs (real store-hub, read-only) 19 passed 0 failed; platform.mjs 199 passed 0 failed; migrations.mjs 76 passed 0 failed 0 skipped => 412 passed, 0 failed. grep for the test secrets across the three hub-SSO run logs: 0, 0, 0. Proof: ~/tasks/multistore-hub/briefs/out/PROOF-LANE-E2.md.
DECISIONS: DECISION MADE — session revocation has NO grace period and no cache for hub-SSO sessions (one extra query per request; ordinary logins keep the 5-minute cache). KNOWN LIMIT flagged: after POST /auth/refresh, authController mints an unmarked token (this lane may not touch authController), so that rotated session returns to the cached path. DECISION MADE — HUB_ORIGIN is REQUIRED while the flag is on (503 when unset or unparseable), stricter than the review's "when set". DECISION MADE — the secret floor is enforced at request time (R7 forbids reading env at import). DECISION MADE — a dedicated in-memory bucket in hubSso.js instead of editing the shared rateLimiter.js. DECISION MADE — ambiguous email is a 409 refusal, never an implicit pick. NOT DONE: the hub-side TOTP step-reuse fix (P1-1) lives in /Users/ludo/store-hub, outside this session's worktree boundary; BLOCKED entry in logs/errors.md.
STATUS: COMPLETE (dashboard side; hub-side P1-1 BLOCKED, see errors.md)
---

---
TIMESTAMP: 2026-09-10 17:40
TASK: Lane F — S1-5 Constants to manifest (12 scope items), branch day1/lane-constants @3fa46a3
BUILT: server/src/config/storeConfig.js — the one request-time read site for store identity (brand, Shopify domain/URL/API version, Whop company id, Triple Whale shop id, Meta Graph version + ad accounts, Frame.io token with legacy-name deprecation, Slack channels, report timezone, product codes) with a documented manifest-adapter seam; GET /api/v1/store-config (session, non-secret) + GET /api/v1/brand (public); 30 server files rewired so no store/brand/channel/ad-account/API-version/domain literal remains (A1: 53 → 3, the 3 explained and pinned); adLauncher refuses (400) without a landing URL; client brand.js reads /api/v1/brand at runtime with the build-time fallback; server/config/env.{PL,MB}.example + A2 snapshots.
TESTED: 81 Lane F tests (23 unit, 43 wiring incl. the A1 tree grep, 6 real-auth route tests incl. the A4 secret scan + negative control, 2 adLauncher, 3 A2 snapshot, 4 client) — all written red first, all green. Existing tests touching the changed files: 13 green (report-tz 18/18, analytics-report-tz 56/56, attribution 138/138, shopify-import 382/382, admin-crud 44/44 …); 6 fail identically on the untouched baseline edc1030 (missing DBs / column / hardcoded main-repo path); golden.mjs needs live creds on both. vite build EXIT=0. One regression found by an existing test (shopify-import n1, 377/5) and fixed red→green (382/0).
OUTPUT: see ~/tasks/multistore-hub/briefs/out/PROOF-LANE-F.md (A1 grep before/after verbatim, every test count, baseline comparison).
DECISIONS: DECISION MADE ×8, listed in docs/lanes/lane-f.md § Deviations — A1 = 3 not 0 (one sanctioned default + two out-of-lane residues, pinned in the test); /api/v1/brand returns null for unset BRAND_* (no brand literal server-side; client keeps the build-time fallback); malformed SHOPIFY_API_VERSION refused not defaulted; no legacy synthesis of PRODUCT_CODES_JSON (pipelines dormant until env is set); PL example leaves Mineblock-only values unset rather than cross-posting; funnelAttribution.js/metaAdsApi.js/staticsGeneration :9373-9378 touched one line each for an item's own acceptance; META_API_VERSION single default moves adsReporting v22→v21 and statics:4592 v23→v21 (pin per store if wanted).
STATUS: COMPLETE (lane deliverable; merge + sandbox env + deploy are the lead's)
---

---
TIMESTAMP: 2026-09-10 17:45
TASK: Lane F2 — S1-5 Constants to manifest, adversarial-review fixes (briefs/out/REVIEW-LANE-F.md: P1-1, P1-2, P2-1, P2-3), branch day1/lane-constants rebased onto hub/main 95fc741
BUILT: Rebased 15 commits onto hub/main (one conflict, logs/progress.md only, both entries kept; staticsGeneration.js and server.js auto-merged). P1-1: PRODUCT_CODES_JSON is now REQUIRED — new StoreConfigError, productCodes() throws on unset/blank/malformed/two-defaults/EMPTY-{}, new assertBootConfig() called as step 0 of server.js start() before any listen, process.exit(1) with a named actionable error; the clickupWebhook wrong-product fall-through (P1 card into PUURE's Frame.io project + a live ClickUp PUT rename) is now unreachable rather than "dormant". P1-2: META_API_VERSION_DEFAULT v21.0 -> v23.0 (newest already in the tree; a single env cannot reproduce the old v21/v22/v23 spread) and the last three hardcoded Graph pins in staticsGeneration.js (:7184, :9737, :10295) now read storeConfig.metaGraphUrl(); env.{PL,MB}.example pin v23.0. P2-1: adsControlCenter.sendSlackAlert() returns when the P&L channel is null instead of POSTing {"channel": null}. P2-3: env.PL.example marks PL's own entry "default": true. Commits e132273, 3ad369d, b53e62d. Pre-rebase tip kept at backup/lane-constants-pre-f2.
TESTED: Red before green on every behavioural fix. RED: boot-gate.mjs 8 tests -> 1 pass 7 fail, and the spawned real server with PRODUCT_CODES_JSON unset printed "Server running on port 39751 in test mode" (it booted — the fail-open reproduced); review-fixes.mjs 8 tests -> 1 pass 7 fail; the Slack path reproduced verbatim as "RED: slack calls made = 1 | channel value posted = null"; the A1 grep for graph.facebook.com/v[0-9] in server/src returned 3 lines; PRODUCT_CODES_JSON={} returned {} with no throw. GREEN: identical checks — real boot with the key unset EXIT=1 and "STORE CONFIG INVALID — refusing to start: PRODUCT_CODES_JSON is not set…", real boot with MB's value "Store config OK (PRODUCT_CODES_JSON)" then "Server running on port 39756"; boot-gate 8/8; review-fixes 8/8; graph-version grep count 0. Edge cases: unset, blank, '{nope', '[]', '{}', wrong shape, two defaults, malformed META_API_VERSION, and a throw escaping into the ClickUp webhook (caught by the route's own try at clickupWebhook.js:1086 — error response, no ClickUp write, no crash).
OUTPUT: Lane F suite 97/97 (was 81/81; +16 new tests, 0 regressions) across store-config 23, snapshot 3, routes 6, ad-launcher-landing 2, wiring 43, boot-gate 8 (new), review-fixes 8 (new), client brand-runtime 4. Existing suites 814/814: shopify-import 382, report-tz 18, analytics-report-tz 56, attribution/engine 138, metrics/routes 144, migrations 76. Grand total 911 tests, 911 pass, 0 fail. A1/R15 grep unchanged at the 3 residues the reviewer accepted; graph.facebook.com version literals 0 (was 3). A2 snapshot diff is exactly three lines: meta.apiVersion v21.0->v23.0 on both stores and PL.productCodes.PL.default false->true. Full verbatim output: ~/tasks/multistore-hub/briefs/out/PROOF-LANE-F2.md.
