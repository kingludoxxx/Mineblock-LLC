...
TESTED: (1) earthbreeze: rank=2, r3=3, v7=1, v21=1 (slipped 1 rank, correct). (2) norseorganics: rank=1, r3=1, v7=0, v21=0 (stable top position, correct). (3) try-forge: rank=1, r3=34, v7=33 (climbed 33 ranks, correct). (4) thegreatproject: rank=1, r3=34, v7=33; rank=2, r3=41, v7=39; all top-5 BANGERs with meaningful velocity (33-39 range). (5) Main ads API /brands/:id/ads?sort=rank_asc returns velocity in all 4 brands. (6) Debug endpoint removed cleanly — confirmed not routing.
OUTPUT: All 4 brands have non-null rank_3d/rank_7d/rank_21d/velocity_7d/velocity_21d. Values are meaningful: earthbreeze stable (v7=1), norseorganics stable top (v7=0), try-forge and thegreatproject both showing 33-39 position climbers as BANGERs.
DECISIONS: Used halfWidthDays=centerDays (lower=0) so velocity data is available from day 1 of scraping. Interpretation: d3/d7/d21 all converge to oldest available snapshot until enough history accumulates. Day 3+: d3 finds genuine 3-day delta. Day 7+: d7 genuine. Day 21+: d21 genuine. This is correct behavior — meaningful velocity from first pair of scrapes. DECISION MADE.
STATUS: COMPLETE
---
TIMESTAMP: 2026-05-27 19:13
TASK: Brand Spy — SPRTN-style brand detail overview (commit 7a40bb8)
BUILT: Complete rewrite of BrandDetail.jsx (679→1136L) to match SPRTN/Adria competitor spy tool reference design. Overview tab: 3-column ad card grid with thumbnail previews, play button overlay on videos (clicking opens videoUrl in new tab), format badges (VIDEO/IMAGE/CAROUSEL), tier badges, date range overlays, hook text, active-days badge, and IntelDrawer on card click. Media mix bar: Video/Image/Carousel counts with proportional color bar. AI Intel panel: 6 categories (Personas, Ad Angles, USPs, Desires, Emotions, Themes), loaded lazily from new backend endpoint. Time filters: All time | 7d | 30d | 90d | 180d. Sort: Top rank | Most recent | Longest running | Climbing fast. Tier filter pills and pages dropdown preserved. Intelligence tab: existing rank/velocity table preserved intact. Other tabs (Hooks, Ad Copy, Headlines, Landing Pages): "coming soon" stubs. Backend: new GET /brands/:id/intel endpoint fetches top 60 active ads, calls Claude Haiku (claude-haiku-4-5-20251001), returns {personas, adAngles, usps, desires, emotions, themes}. brandSpyDb.js: added minStartDate WHERE clause to listAds for time-filter support. brandSpy.js: passes minStartDate when ?days=N query param is present.
TESTED: (1) Vite build clean — 2464 modules, 0 errors, 0 warnings. (2) Intel endpoint live at /brands/f304cc1a.../intel — returned 7 items per category for earthbreeze.com with accurate AI analysis (eco-conscious parents, plastic-free messaging, etc.). (3) Invalid brand ID returns {'error': 'Brand not found'} (404). (4) days=1 filter returns total=0 (correct — no ads launched in last 24h for norseorganics). (5) sort=first_seen_desc returns ads ordered correctly by firstSeenAt. (6) Deploy dep-d8bk328jo6nc7381k820 confirmed LIVE at 19:12:54Z on srv-d6qavvf5gffc73em69n0.
OUTPUT: Intel endpoint returns real AI-generated analysis. Days filter works correctly. Sort works. Build passes. Render deploy live.
DECISIONS: Preserved Intelligence tab (existing rank/velocity table) as a second tab rather than removing it — keeps existing power-user functionality. Time filter passes days=N to server-side minStartDate filter rather than client-side filtering, for accuracy with large datasets. Intel loads lazily (only when Overview tab first viewed) to avoid unnecessary API calls. DECISION MADE: intelFetched flag prevents redundant Claude calls across tab switches.
STATUS: COMPLETE
---
...
TESTED: (1) Hard refreshed /app/assets — card shows $69 (was $249). (2) Opened Edit → confirmed "Ad Angles" section open by default showing ANGLE 1/2 content. (3) Opened Offers & Promotions → confirmed BASE PRICE (SHOWN IN ADS) label appears first with $69 value. (4) Live API call confirmed winning_angles=11304 chars, angles_count=8, price=$69, bundle_variants populated, discount_codes=MINER10. (5) Traced full client→server→prompt pipeline: all profile fields correctly camelCase-mapped and reaching buildClaudePrompt; angle_data from seeded angles triggers rich ANGLE STRATEGY box in Claude prompt.
OUTPUT: Product Library shows $69 on card and in editable Base Price field. Ad Angles section open by default with all 6 angle profiles visible. Offers & Promotions has Base Price as first editable field. Pipeline audit: clean — no missing fields, no name mismatches in the critical path.
DECISIONS: Stored angle intelligence in winning_angles TEXT field (not a new column) since it's already mapped to Claude prompt. Price set to $69 (single unit) not $69.99 per user instruction. winning_angles removed from Brand Voice section to avoid duplication.
STATUS: COMPLETE
---
TIMESTAMP: 2026-05-17 22:15
TASK: Live end-to-end generation test — verify product library data flows to AI
BUILT: No code changed. Full audit of the live generation pipeline.
TESTED: Triggered a generation on the Statics Generation page: Miner Forge Pro + Anti-Fake / Competitor Callout angle + Bold Claim template (phone screenshot). 4×200 POST responses to /api/v1/statics-generation/generate confirmed generation fired. Fetched the resulting card (id: 21e50960) from /api/v1/statics-generation/creatives/pipeline and /api/v1/statics-generation/creatives/:id. Opened the generated image directly to inspect output.
OUTPUT: Generated image shows sticky note with text "this $69 miner just hit a full block. just hit a full block. checked my wallet morning. not a joke." alongside a Bitcoin wallet showing 3.123 BTC. CONFIRMS: (1) $69 price from product library reached the image generation correctly. (2) Anti-Fake / Competitor Callout angle was applied (testimonial tone, accidental-winner framing). (3) Validation score 76/100 (layout_match 92, product_fidelity 88, competitor_branding 100). Full data-flow traced: product_profiles.winning_angles → profile.winningAngles → Claude prompt bullet "Winning Ad Angles: [11,304 chars]" ✅. product_profiles.price=$69 → productPrice → POST body product.price → Claude prompt ✅. selectedAngleData → angle_data POST field → buildClaudePrompt() ANGLE STRATEGY box ✅.
DECISIONS: Two issues found but NOT introduced by our changes: (1) generation_provider=nanobanana — Gemini failed for this run and fell back to NanoBanana (likely transient quota/API issue; server default IS Gemini at line 976). (2) generated_copy: null for all 87 recent review cards — Claude adapted_text (headline/body/CTA) is not being persisted. Root cause: when NanoBanana redirect path completes, claudeAnalysis in status response appears null, so client saves adapted_text=null → generated_copy=null → only the post-save validation jsonb ({validation:...}) ends up in claude_analysis. These are pre-existing bugs. Core data flow (price, angle, winning_angles, angle_data) is confirmed working.
STATUS: COMPLETE
---
