...
export async function loadMemberships(exec = pgQuery) {
  await ensureFunnelCostsTables();
  return exec(`
...
export async function loadCostIndex(exec = pgQuery) {
  const [rates, members] = await Promise.all([loadRates(exec), loadMemberships(exec)]);
...
export async function refreshCoverage(scope, refId, exec = pgQuery) {
  ...
  // coverage off a snapshot that predates the write. loadCostIndex (not bare
  // loadRates) so the membership ledger rides along — half an index silently
  // reverts group resolution to "as of now", the restatement bug it prevents.
  const rateIndex = await loadCostIndex(exec);
  const today = dayKey();
  let flipped = 0;
