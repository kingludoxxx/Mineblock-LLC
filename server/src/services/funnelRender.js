  ...
  switch (t) {
    ...
    case 'order_confirmation': {
      ...
      return (
        ...
        `<h3 class='lb-orderconf-title'>${title}</h3>` +
        `<div class='fos-order-summary' data-fos-order-summary>` +
        `<div class='fos-os-empty'>Your order details will appear here.</div></div>` +
        `<p class='lb-orderconf-note'>${note}</p>` +
        `</section>`
      );
    }
    case 'optin_form': {
...
fetch(API+'/session/'+encodeURIComponent(SID),{credentials:'same-origin'}).then(function(r){return r.json().catch(function(){return {};}).then(function(j){return {status:r.status,json:j};});}).then(function(res){if(res.status!==200||!res.json||!res.json.success||!res.json.data){return;}fill(res.json.data);}).catch(function(){});
}catch(e){}});
})();`;
