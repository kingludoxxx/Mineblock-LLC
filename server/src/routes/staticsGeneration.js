...
import { buildClaudePrompt, buildDocumentClaudePrompt, buildNanoBananaPrompt, buildSwapPairs, buildLayoutAnalysisPrompt, buildIterationPrompt, buildIterationNanoBananaPrompt, buildProductSwapPrompt, buildTextPolishPrompt, buildComparisonConfig } from '../utils/staticsPrompts.js';
...
router.post('/generate', authenticate, async (req, res) => {
  ...
  try {
    ...
    // ── Phase 3+4: COMPARISON PATH — dual Gemini product images + Playwright renderer ──
    // For comparison ("us vs them") templates, bypass the reference-editing approach entirely.
    // Instead: generate two product images via Gemini (clean premium LEFT, degraded knockoff RIGHT),
    // then render the full comparison layout from scratch with Playwright/HTML. Zero template pollution.
    if (isComparisonTemplate) {
      console.log(`[staticsGeneration] 🆚 Phase 3+4: Starting dual-Gemini + Playwright comparison render`);
      try {
        // ── 3a. Build comparison config (right-column header, product descriptions) ──
        const cc = buildComparisonConfig(product, templateData, angle_data || null);
        console.log(`[staticsGeneration] 🆚 Comparison config: rightHeader="${cc.rightColumnHeader}", type="${cc.comparisonType}"`);

        // ── 3b. Fetch the product image as base64 for Gemini inline input ──
        async function fetchProductBase64(url) {
          if (!url) return null;
          ...
          if (!resp.ok) throw new Error(`Failed to fetch product image: ${resp.status} ${fetchUrl.slice(0, 100)}`);
          const buf = Buffer.from(await resp.arrayBuffer());
          const mimeType = detectMime(buf);
          return { base64: buf.toString('base64'), mimeType };
        }

        const productImageData = await fetchProductBase64(product.product_image_url);
        if (!productImageData) {
          throw new Error('No product image available for comparison render — cannot proceed without product photo');
        }
        console.log(`[staticsGeneration] 🆚 Product image fetched for Gemini (${productImageData.mimeType}, ${productImageData.base64.length} base64 chars)`);

        // ── 3c. Build Gemini prompts for LEFT (premium) and RIGHT (knockoff) images ──
        const leftPrompt = `Professional product photography: render the ${product.name} Bitcoin miner from the input product image as a clean studio product shot. WHITE background only. The product centered, sharp, crisp, premium quality — all details (display, ports, housing) clearly visible. NO text. NO logos. NO layout elements. Just the clean product on white.`;

        const rightPrompt = `Create a KNOCKOFF Bitcoin miner product image. Use the input image as a reference for the FORM FACTOR only (keep the same general shape and size category). Make it look cheap and fake: slightly blurry/soft focus, desaturated gray-black colors, dull matte plastic with visible seams, BLACK/blank screen (no glow, no display), no visible brand markings, no quality ports, AliExpress-aesthetic gadget feel. WHITE background. Centered. NO text. This represents a generic knockoff in the same product category.`;

        storeGeminiResult(earlyTaskId, { status: 'processing', progress: 'Generating comparison product images (dual Gemini)...' });

        // ── 3d. Run both Gemini image generation calls in parallel ──
        const productImages = [productImageData];
        const [leftResult, rightResult] = await Promise.allSettled([
          editImage(leftPrompt, productImages, '1:1'),
          editImage(rightPrompt, productImages, '1:1'),
        ]);

        const leftOk  = leftResult.status  === 'fulfilled';
        const rightOk = rightResult.status === 'fulfilled';
        console.log(`[staticsGeneration] 🆚 Gemini LEFT: ${leftOk ? 'ok' : 'failed — ' + leftResult.reason?.message}`);
        console.log(`[staticsGeneration] 🆚 Gemini RIGHT: ${rightOk ? 'ok' : 'failed — ' + rightResult.reason?.message}`);

        if (!leftOk && !rightOk) {
          throw new Error(`Both Gemini product image calls failed. Left: ${leftResult.reason?.message}. Right: ${rightResult.reason?.message}`);
        }

        const leftImgBase64  = leftOk  ? leftResult.value.buffer.toString('base64')  : '';
        const leftImgMime    = leftOk  ? leftResult.value.mimeType  : 'image/png';
        const rightImgBase64 = rightOk ? rightResult.value.buffer.toString('base64') : '';
        const rightImgMime   = rightOk ? rightResult.value.mimeType : 'image/png';

        // ── 3e. Parse copy from claudeResult.adapted_text ──
        let adaptedText = claudeResult?.adapted_text;
        if (typeof adaptedText === 'string') {
          try { adaptedText = JSON.parse(adaptedText); } catch { adaptedText = { headline: adaptedText }; }
        }
        if (!adaptedText || typeof adaptedText !== 'object') adaptedText = {};

        const compHeadline = (adaptedText.headline || '').trim() || 'FAKE MINERS ARE CANCELED';
        const footnote     = (adaptedText.cta || adaptedText.footnote || '').trim();

        // Split bullets: if any item has '|', split on that; otherwise first-half / second-half
        let rawBullets = adaptedText.bullets ?? adaptedText.bullet ?? [];
        if (typeof rawBullets === 'string') rawBullets = rawBullets.split('\n').map(b => b.trim()).filter(Boolean);
        if (!Array.isArray(rawBullets)) rawBullets = [];

        let leftBullets, rightBullets;
        const hasPipe = rawBullets.some(b => String(b).includes('|'));
        if (hasPipe) {
          leftBullets  = rawBullets.map(b => String(b).split('|')[0].trim()).filter(Boolean).slice(0, 4);
          rightBullets = rawBullets.map(b => (String(b).split('|')[1] || '').trim()).filter(Boolean).slice(0, 4);
        } else {
          const mid    = Math.ceil(rawBullets.length / 2);
          leftBullets  = rawBullets.slice(0, mid).slice(0, 4);
          rightBullets = rawBullets.slice(mid).slice(0, 4);
        }

        // Fallback bullets if Claude produced nothing useful
        if (leftBullets.length === 0)  leftBullets  = ['High hash rate', 'Energy efficient', 'Proven reliability', 'Quality components'];
        if (rightBullets.length === 0) rightBullets = ['Low performance', 'High power waste', 'Frequent failures', 'Cheap materials'];

        const comparisonOptions = {
          headline:         compHeadline,
          leftHeader:       product.name,
          rightHeader:      cc.rightColumnHeader,
          leftBullets,
          rightBullets,
          footnote,
          leftImageBase64:  leftImgBase64,
          rightImageBase64: rightImgBase64,
          leftImageMime:    leftImgMime,
          rightImageMime:   rightImgMime,
        };

        // ── Phase 4: Render all ratios with Playwright ──
        storeGeminiResult(earlyTaskId, { status: 'processing', progress: 'Rendering comparison layout (Playwright)...' });

        const { renderComparison } = await import('../services/playwrightRenderer.js');
        ...
        const ratioResults = await Promise.allSettled(
          ratiosToGenerate.map(async (r) => {
            ...
            const pngBuffer = await renderComparison(comparisonOptions, dims);

            let finalBuffer = pngBuffer;
            // Apply brand logo watermark (same as other paths)
            ...
            let resultImageUrl;
            if (isR2Configured()) {
              const r2Key = `statics-comparison/${crypto.randomUUID()}.png`;
            ...
            const taskId = `comparison-${crypto.randomUUID()}`;
            storeGeminiResult(taskId, {
              status: 'completed',
              resultImageUrl,
              provider: 'playwright',
              model: 'playwright-comparison',
              claudeAnalysis: claudeResult || null,
              quality_warning: visionWarning || null,
            });
            console.log(`[staticsGeneration] 🆚 ✅ Comparison ${r}: ${taskId}`);
            return { taskId, ratio: r };
          })
        );
        ...
        ratioResults.filter(r => r.status === 'rejected').forEach((r, i) =>
          console.error(`[staticsGeneration] 🆚 Comparison ${ratiosToGenerate[i]} Playwright failed: ${r.reason?.message}`)
        );
        ...
        if (tasks.length > 0) {
          ...
          console.log(`[staticsGeneration] 🆚 earlyTaskId ${earlyTaskId} → completed (comparison Playwright, ${tasks.length} ratio(s))`);
          return;
        }

        console.warn(`[staticsGeneration] 🆚 Comparison Playwright failed all ratios — falling through to normal Gemini path`);
      } catch (compErr) {
        console.error(`[staticsGeneration] 🆚 Comparison Phase 3+4 failed — falling through to normal Gemini path: ${compErr.message}`);
      }
    }
