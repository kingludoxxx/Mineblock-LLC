...
try {
  ...
  ok(ownerRoles.length === 1 && ownerRoles[0] === FULL,
    `J1: hub owner -> ${FULL}`, `got ${JSON.stringify(ownerRoles)}`);
  const fullPerms = await permsOf(FULL);
  ok(grants(fullPerms, 'dashboard'),
    'J1: that role really opens the home page (dashboard:access)', `perms=${JSON.stringify(fullPerms)}`);
  ok(grants(fullPerms, 'orders'), 'J1: and the revenue pages a store owner needs (orders:access)');
  ...
  ok(again.status === 302 && (await rolesOf(owner.email)).join() === FULL,
  ...
  ok(Boolean(dv) && dv.mapped_role === FULL && Array.isArray(dv.held_roles) && dv.held_roles.join() === READ_ONLY,
  ...
  await q('UPDATE hub_role_map SET dashboard_role = $1 WHERE hub_role = $2', [FULL, 'owner']);
