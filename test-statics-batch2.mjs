#!/usr/bin/env node
// Batch 2: Test more reference images to identify patterns
...
async function pollStatus(token, taskId) {
  ...
  while (Date.now() - start < 300000) {
    if (Date.now() - start > 800000) token = await login();
...
async function main() {
  ...
  const tests = [
    { ref: 'https://xn7q-nefz-qhlj.n7d.xano.io/vault/r4fM_Vz3/3ADJaAVyHseGn0cKPzimGrRfRAY/DDWUqw../4408.webp', angle: 'Value Proposition', label: 'test4-value-4408' },
    { ref: 'https://xn7q-nefz-qhlj.n7d.xano.io/vault/r4fM_Vz3/Jb-hns7qoAdcbwzmBbDJLeHi0G8/2Ll3XA../3643.webp', angle: 'Curiosity', label: 'test5-curiosity-3643' },
    { ref: 'https://xn7q-nefz-qhlj.n7d.xano.io/vault/r4fM_Vz3/wUaXxuwXjSy7QrOuK88lh8urdV4/1K7dHA../1983.webp', angle: 'Social Proof', label: 'test6-social-1983' },
    { ref: 'https://xn7q-nefz-qhlj.n7d.xano.io/vault/r4fM_Vz3/DJ6lbSosi847hcS_hWlxrdUXEDA/RCSP2Q../3732.webp', angle: null, label: 'test7-noangle-3732' },
    { ref: 'https://xn7q-nefz-qhlj.n7d.xano.io/vault/r4fM_Vz3/qoP-YXSUZLxCurLVpS5ahLhtpNE/1yGPUw../3865.webp', angle: 'Urgency/Scarcity', label: 'test8-urgency-3865' },
  ];

  for (let i = 0; i < tests.length; i++) {
    const tc = tests[i];
    console.log(`\n--- Test ${i+4}/${tests.length+3}: ${tc.label} ---`);
    try {
      token = await login();
      ...
      if (!genResult.success) { console.error('  ❌ Generate error'); continue; }
      ...
      console.log(`  People: ${analysis?.people_count}, Logo: ${analysis?.has_competitor_logo}, Orientation: ${analysis?.product_orientation}`);
      console.log(`  Headline: "${(analysis?.original_text?.headline||'').slice(0,50)}" → "${(analysis?.adapted_text?.headline||'').slice(0,50)}"`);
      console.log(`  Swaps (${swapPairs.length}):`);
      swapPairs.forEach((sp, idx) => console.log(`    ${idx+1}. [${sp.field}] "${sp.original?.slice(0,40)}" → "${sp.adapted?.slice(0,40)}"`));
      ...
      if (taskId) {
        ...
        try { await downloadImage(tc.ref, `${tc.label}-reference.webp`); } catch {}
      }
    } catch (err) {
